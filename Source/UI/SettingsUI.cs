    ...
    public static class SettingsUI
    {
        ...
        public static void DrawTTSSettings(Rect inRect, TTSSettings settings)
        {
            ...
            if (Widgets.ButtonText(supplierRect, supplierDisplay))
            {
                ...
                options.Add(new FloatMenuOption("MiMo TTS (小米)", delegate
                {
                    settings.Supplier = TTSSettings.TTSSupplier.MiMo;
                    TTSService.SetProvider(settings.Supplier, settings);
                }));
        ...
        private static void DrawProcessingPromptSection(Listing_Standard listing, TTSSettings settings)
        {
            ...
            // Reset buttons - Third row: MiMo + Custom
            if (settings.Supplier == TTSSettings.TTSSupplier.MiMo)
            {
                listing.Gap(6f);
                Rect resetButtonsRect3 = listing.GetRect(30f);
                Rect miMoRect = new Rect(resetButtonsRect3.x, resetButtonsRect3.y, btnW, resetButtonsRect3.height);
                if (Widgets.ButtonText(miMoRect, "重置 MiMo 提示词"))
                {
                    settings.CustomTTSProcessingPrompt = Data.TTSConstant.DefaultTTSProcessingPrompt_MiMo;
                    processingPromptBuffer = Data.TTSConstant.DefaultTTSProcessingPrompt_MiMo;
                }
            }
            if (settings.Supplier == TTSSettings.TTSSupplier.Custom)
            {
        ...
        private static string SupplierString(TTSSettings.TTSSupplier supplier)
        {
            ...
            return supplier switch
            {
                ...
                TTSSettings.TTSSupplier.MiMo => "MiMo TTS (小米)",
